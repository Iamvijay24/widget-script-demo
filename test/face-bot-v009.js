...
(function () {
    ...
    let widgetIconHide = true;  // Default to hidden
    let toggleChatFn;
    let chatButton;
    let chatContainer; // Declare chatContainer here
    ...
    const initializeChatWidget = () => {
        const setupWidget = () => {
            ...
            const defaultVideoURL = `https://d1jcbfcd9v1do3.cloudfront.net/${fb_partner_id}/videos/knowledge_base/${fb_id}/default/1_playlist.m3u8`;
            let bgColor = configurationData?.theme_color ?? "#034b83"
            let title = configurationData?.facebot_title ?? "AI assistant 24/7";

            const askContactVideoUrl = `https://d1jcbfcd9v1do3.cloudfront.net/${fb_partner_id}/videos/knowledge_base/${fb_id}/contact/contact_playlist.m3u8`;

            const getInformationFromUser = configurationData?.contact_prompt_point ?? null;
...
<a style="margin: 0; font-size: 5px; position: absolute; bottom: 3px; right: .6rem; text-decoration: none;" href="https://face-bot.ai/" target="_blank" id="fb_logo">
    ...
    <span style="color: #034b83;">Face-Bot.ai</span>
            ...
            // Initially hide the chat button if widgetIconHide is true
            if (widgetIconHide) {
                chatButton.style.display = 'none'; // or 'none'
            }
...
`;
            ...
            // Define the toggleChatFn inside setupWidget so it can access chatContainer
            toggleChatFn = async () => {
            ...
            document
                ...
                .addEventListener("click", toggleChatFn);
                ...
                if (!isRecording) {
                    try {
                        ...
                        recorder = RecordRTC(stream, {
                            type: "audio",
                            mimeType: "audio/wav",
                            recorderType: isSafari ? StereoAudioRecorder : null,
                        });
                        ...
                        const notificationSound = new Audio("/test/pop-alert.mp3");
        ...
        const fetchConfiguration = async () => {
            try {
                ...
                if (data && data.data) {
                    ...
                    console.log("Configuration:", configurationData);

                    widgetIconHide = configurationData?.active === false;
                    ...
                    console.log("No configuration data received or invalid data structure.");
                    chatButton.style.display = 'none';
                }
            } catch (error) {
